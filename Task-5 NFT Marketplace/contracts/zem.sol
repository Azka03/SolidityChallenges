...
contract ZemToken  is IERC20 {
    ...
    function mint (uint _value) external returns (bool) {
        // console.log("INITIAL FROM CONTRACT", _balanceOf[contractAddress]);

        // require(msg.sender == owner);
        _totalSupply = _totalSupply + _value;
        // console.log("Total Supply", _totalSupply);

        _balanceOf[contractAddress] = _totalSupply; //_balanceOf[_to] += _value
        // console.log("FROM CONTRACT", _balanceOf[contractAddress]);
    ...
    function approve(address _spender, uint _value) external override returns (bool) {
        // console.log("KON BANDA HAI? ", msg.sender);
